...
def download_dataset(url, destination):
    print(f" Iniciando descarga desde el servidor remoto...")
...
def ensure_raw_dataset():
    ...
    print(f"Alerta: El CSV crudo no se encuentra en la ruta habitual.")

    if os.path.exists(COMPRESSED_FILE):
        print(f"Se localizó un respaldo comprimido local.")
        decompress_dataset(COMPRESSED_FILE, RAW_DIR)
        return

    print(f"No se encontraron datos locales. Iniciando protocolo de recuperación.")
...
def run_extract():
    ...
    df = pd.read_csv(RAW_FILE, usecols=COLUMNS_TO_KEEP, dtype=DTYPE_MAP)
